    ...
    <div className="space-y-16 py-10">
      <section className="text-center max-w-3xl mx-auto space-y-6">
        <h1 className="text-5xl font-extrabold tracking-tight text-gray-900 sm:text-6xl">
          Build Perfect <span className="text-indigo-600">Multi-Step</span> Experiences
        </h1>
        <p className="text-xl text-gray-600 leading-relaxed">
          A flexible, headless wizard stepper for React. Persistence, validation, and complex flows made simple.
        </p>
        <div className="flex items-center justify-center gap-4 pt-4">
          <Button size="lg" onClick={() => window.open('https://github.com/ZizzX/wizzard-stepper-react', '_blank')}>
            Get Started
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        {features.map((feature) => (
          <Link key={feature.path} to={feature.path} className="group transition-transform hover:-translate-y-1">
            <Card className="h-full border-gray-100 hover:border-indigo-200 hover:shadow-md transition-all">
              <CardHeader className="flex flex-row items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${feature.color}`}>
                  {feature.icon}
                </div>
                <div>
