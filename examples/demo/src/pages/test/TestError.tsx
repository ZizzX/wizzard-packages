...
class SimpleAdapter implements IValidatorAdapter<ErrorData> {
  private rules: Record<
    string,
    (val: unknown) => string | null | Promise<string | null>
  >;
  constructor(
    rules: Record<
      string,
...
const BasicInfoStep = () => {
  ...
  const { data, allErrors, handleStepChange, activeSteps, currentStep } =
    useWizard<ErrorData>();
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">
        Try 'taken-username' to trigger error.
      </p>
    ...
    <div data-testid="wizard-container" className="max-w-md mx-auto py-8">
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        {activeSteps.map((s, i) => {
          ...
          const isCompleted =
            i < activeSteps.findIndex((step) => step.id === currentStep?.id) &&
            !hasError;
