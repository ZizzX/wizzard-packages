...
function SimpleStepper() {
    ...
    const progress = ((activeSteps.findIndex(s => s.id === currentStepId) + 1) / (activeSteps.length || 1)) * 100;
        ...
        <div className="mb-8">
            ...
            <div className="flex justify-between">
                {activeSteps.map(step => (
                    <div key={step.id} data-testid="step-indicator" className={`text-sm ${step.id === currentStepId ? 'font-bold' : ''}`}>
                        {step.label}
                    </div>
                ))}
            </div>
