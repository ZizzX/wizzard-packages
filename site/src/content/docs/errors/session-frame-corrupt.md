...
Every frame of a recording is a `WizardState` the engine committed. This one is not, in one of two
...
Neither can come out of a running wizard, so the recording was damaged after it was made: cut
...
it, the hook has to leave `stack` and `visited` as it found them.
