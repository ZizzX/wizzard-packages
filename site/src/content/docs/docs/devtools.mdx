...
`devtools()`, `recordSession`, the bundle types and the panel itself are public API under
semver - a recording attached to a bug report stays readable across minor releases, which is the
point of it.

Three helpers the panel and this site's pages are drawn with are not: `layoutGraph`, which gives
a graph coordinates, `formatExpr`, which shortens an expression to a label, and `diffState`,
which lists what changed between two states, along with the types they use - `PositionedGraph`,
`Positioned` and `Change`. They are exported because the docs needed them, and they may change
in a minor release. Use them for a tool you can rebuild, not for something a user depends on.
