...
should be, `order` holding a number - makes one of those reads throw. The validator returns that
as [`flow-unreadable`](../../errors/flow-unreadable/) rather than throwing, but the problem it
returns names a read, not the field a service got wrong. Checking `id` and `steps` yourself first
is what turns the common cases into problems that say what to fix.
...
A patch that introduces a group step without a traversal installed throws
[`groups-not-installed`](../../errors/groups-not-installed/) rather than returning `false`. That is a wiring mistake in your application, not an outcome of the payload.
