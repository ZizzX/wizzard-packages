...
test.describe('State Persistence', () => {
  ...
  test('should clear storage on reset', async ({ page }) => {
    ...
    let keys = await page.evaluate(() => Object.keys(localStorage).filter(k => k.includes('wizard')));
  ...
  test('should restore visited steps state', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="breadcrumb-step-1"]')).toHaveClass(/visited|completed/);
    await expect(page.locator('[data-testid="breadcrumb-step-2"]')).toHaveClass(/visited|completed/);
  });
