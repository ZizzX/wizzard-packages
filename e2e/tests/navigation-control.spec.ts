...
test.describe('Navigation Control', () => {
    ...
    test('should allow navigation only to visited steps in visited mode (default)', async ({ page }) => {
        // Initially on step-1
    ...
    test('should respect step-level canNavigateTo function', async ({ page }) => {
        ...
        // Should still be on step-1 (blocked by canNavigateTo)
        await expect(page.locator('[data-testid="current-step"]')).toContainText('Step 1');

        // Fill step1Data
        await page.fill('[data-testid="step1-input"]', 'John Doe');

        // Navigate to step-3 first to mark it as completed
        await page.click('[data-testid="breadcrumb-step-3"]');
        await expect(page.locator('[data-testid="current-step"]')).toContainText('Step 3');

        // Now can navigate to step-4
        await page.click('[data-testid="breadcrumb-step-4"]');
    ...
    test('should update breadcrumb status correctly', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="breadcrumb-step-1"]')).toHaveAttribute('data-status', 'current');
        ...
        await expect(page.locator('[data-testid="breadcrumb-step-1"]')).toHaveAttribute('data-status', /visited|completed/);
        await expect(page.locator('[data-testid="breadcrumb-step-2"]')).toHaveAttribute('data-status', 'current');
        ...
        await expect(page.locator('[data-testid="breadcrumb-step-2"]')).toHaveAttribute('data-status', /visited|completed/);
        await expect(page.locator('[data-testid="breadcrumb-step-3"]')).toHaveAttribute('data-status', 'current');
    });
