...
test.describe('Array Data Manipulation', () => {
  ...
  test('should update item in array', async ({ page }) => {
    ...
    // Verify update
    await expect(page.locator('[data-testid="array-item"]:first-child')).toContainText('Updated Item');
    await expect(page.locator('[data-testid="array-item"]:first-child')).not.toContainText('Original Item');
  });
  ...
  test('should persist array data on page reload', async ({ page }) => {
    ...
    const count = await page.locator('[data-testid="array-item"]').count();
    expect(count).toBe(1);
    await expect(page.locator('[data-testid="array-item"]:first-child')).toContainText('Persisted Item');
  });
