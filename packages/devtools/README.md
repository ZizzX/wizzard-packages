...
`devtools`, `recordSession` and the bundle types are the stable surface here. The entry also
exports `layoutGraph`, `formatExpr` and `diffState`, and the package root exports
`FlowGraphView` and `EDGE_DRAW_CAP`: those five exist for a host drawing its own panel and may
