...
> **Path**\<`T`\> = `T` _extends_ `ReadonlyArray`\<infer V\> ? `IsTuple`\<`T`\> _extends_ `true` ? `{ [K in TupleKeys<T>]-?: PathImpl<K & string, T[K]> }`\[`TupleKeys`\<`T`\>\] : `PathImpl`\<`number`, `V`\> : `{ [K in keyof T]-?: PathImpl<K & string, T[K]> }`\[keyof `T`\]
