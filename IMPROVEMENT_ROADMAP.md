...
/packages
  ...
  /ui                # docs site + interactive examples
```
...
- [ ] Создать `packages/ui` и базовую структуру приложения
- [ ] Подключить API docs из `/docs/api`
- [ ] Добавить базовые маршруты (главная, API, примеры)
- [ ] Настроить сборку и билд-артефакт для публикации

**Критерии завершения:**

- [ ] Пакет `ui` собирается отдельно
- [ ] API docs отображаются внутри UI
- [ ] Документационный сайт собирается локально без ошибок
